...
Claude Code user configuration is git-tracked under ~config/claude/~.
~modules/home/claude-code.nix~ symlinks it into ~~/.claude/~ so settings and
agents are version-controlled and writable.

Sub-agent definitions live in ~config/claude/agents/~ (one ~.md~ file per
agent). Adding a new file there takes effect immediately — no NixOS rebuild
needed.
...
just deploy syncthing-pve root@sync.stfl.dev
#+end_src
...
An easy solution is to do a remote setup - have the nixos config on an existing
machine and connect to the new system via ssh.
...
Do partitioning according to the host specific instructions
=TODO= use disko
...
identify supported citrix versions

#+begin_src bash
nix search nixpkgs citrix_workspace --no-pretty
#+end_src

Download the Linux Desktop version as tar.gz from https://www.citrix.com/downloads/workspace-app/

update the version in [[file:modules/citrix.nix]]
