...
auth include login
#+end_quote
** TODO sway on Proxmox VE Workstation

sway from nix does net seem to work on PVE.
>>> ==install sway from Debian repo and start that...=
>> ~individual packages will require nixGL wrapping instead of wrapping sway directly!!!!~
...
#+begin_src bash
/usr/bin/sway --debug | tee $> /tmp/sway.log
#+end_src
